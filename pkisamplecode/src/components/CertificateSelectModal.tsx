...
interface CertificateSelectModalProps {
  ...
  title?: string;
  description?: string;
  challengeEndpoint?: string;  // 챌린지 요청 엔드포인트
  verifyEndpoint?: string;     // 서명 검증 엔드포인트
  skipVerify?: boolean;        // 서명 검증 생략 (서명만 반환)
}

type Step = 'loading' | 'no-agent' | 'select' | 'signing' | 'success' | 'error';

export default function CertificateSelectModal({
  isOpen,
  onClose,
  onSelect,
  title = '인증서 선택',
    ...
    <div style={modalStyle} onClick={handleClose}>
      <div style={contentStyle} onClick={(e) => e.stopPropagation()}>
        ...
        <div style={{ padding: '24px 24px 0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <h2 style={{ fontSize: '20px', fontWeight: 'bold', color: '#111827' }}>{title}</h2>
        ...
        <div style={{ padding: '24px' }}>
          ...
          {step === 'no-agent' && (
            <div style={{ textAlign: 'center', padding: '24px 0' }}>
              ...
              <p style={{ color: '#6b7280', fontSize: '14px', marginBottom: '24px' }}>2Check Agent를 설치해주세요.</p>
              <div style={{ display: 'flex', gap: '12px', justifyContent: 'center' }}>
                <a href="/download/agent" style={{ ...buttonStyle, width: 'auto', padding: '12px 24px', textDecoration: 'none' }}>Agent 다운로드</a>
          ...
          {step === 'select' && (
            <>
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '14px' }}>
                    <thead>
                      <tr>
                        ...
                        <th style={{ padding: '12px 8px', textAlign: 'left', borderBottom: '2px solid #e5e7eb', backgroundColor: '#f9fafb' }}>발급자</th>
                    ...
                    <tbody>
                      {certificates.map((cert) => (
                        ...
                        >
                          ...
                          <td style={{ padding: '12px 8px', borderBottom: '1px solid #e5e7eb', color: '#6b7280' }}>{extractCN(cert.issuerDN)}</td>
              ...
              <button onClick={handleSubmit} disabled={!selectedCert || !password} style={{ ...buttonStyle, opacity: (!selectedCert || !password) ? 0.5 : 1 }}>
                확인
              </button>
            </>
          )}
